...
def training_loop(
    ...
    sigma = None, # Differential Privacy sigma
):
    # Initialize.
    ...
    # Add Differential Privacy Hooks
    if sigma is not None:
        print("Adding DP hooks...")
        for parameter in D.parameters():
            def add_noise_to_gradient(grad):
                noise = (1 / batch_size) * sigma * torch.randn(grad.shape).cuda()
                return grad + noise

            if parameter.requires_grad:
                parameter.register_hook(add_noise_to_gradient)
            else:
                print("Skipping parameter without gradient")
