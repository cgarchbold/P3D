...
def calculate_loss(output, target, criterions, weights = [1, 10, 5]):
    ...
    N_protest = int(target['protest'].sum())
    ...
    if N_protest == 0:
        ...
        scores = {}
        scores['protest_acc'] = accuracy_score((outputs[0]).round().detach().cpu(), targets[0].detach().cpu())
    ...
    scores['violence_mse'] = ((outputs[1] - targets[1]).pow(2)).sum() / float(N_protest)
    # mean accuracy for visual attribute for this batch
    comparison = (outputs[2].round() == targets[2])
    comparison.masked_fill_(not_protest_mask.repeat(1, 10),0)
...
def validate(val_loader, model, criterions, epoch):
    ...
    with torch.no_grad():
        ...
        for i, sample in enumerate(val_loader):
            ...
            if N_protest:
                loss_protest.update(losses[0].item(), input.size(0))
                loss_v.update(loss.data.item() - losses[0].item(), N_protest)
            else:
...
def save_checkpoint(state, is_best, filename='checkpoint.pth.tar'):
    ...
    #if is_best:
    shutil.copyfile(filename, 'model_best_dpsgd1_10epoch_nobest.pth.tar')
